...
const SeriesDetailNaytaKids = () => {
        ...
        const fetchBikes = async () => {
            try {
                const { data, error } = await supabase
                    .from('bicicletas')
                    .select('*')
                    .eq('serie_id', 21); // Nayta Kids ID

                if (error) throw error;
                setBikes(data || []);

            } catch (error) {
                console.error('Error fetching Nayta Kids bikes:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading) return <div className="h-screen flex items-center justify-center font-black uppercase tracking-tighter text-2xl">Loading Nayta Kids...</div>;

    const currentBike = bikes[activeIndex] || bikes[0];

    // Specialized specs for Kids
    const specs = currentBike ? [
        { label: 'Talla Rueda', value: currentBike.tamano_rueda || '24"', icon: Activity },
        { label: 'Cuadro', value: currentBike.cuadro_material || 'Aluminio', icon: Circle },
        { label: 'Transmisión', value: currentBike.transmision_modelo || 'Shimano', icon: Cpu },
        { label: 'Seguridad', value: 'Frenos Disco', icon: Anchor },
    ] : [];
                ...
                <div className="w-full relative z-10 mb-12">
                    <div className="relative h-[400px] md:h-[600px] flex items-center justify-center">
                        {bikes.length > 0 ? (
                            ...
                            <div className="text-center text-gray-500 font-bold uppercase tracking-widest">No hay modelos disponibles para Nayta Kids aún.</div>
                        )}
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                    ...
                    <div className="space-y-6 text-left lg:pr-12">
                        <h2 className="text-4xl md:text-5xl font-black text-black leading-tight mb-8">
                            Grandes aventuras <br />
                            para pequeños ciclistas.
                        </h2>

                        <div className="text-lg text-gray-600 leading-relaxed text-left space-y-6">
                            <p>
                                La Nayta Kids está diseñada para que los más jóvenes descubran la libertad de los senderos. Con un cuadro ligero y componentes adaptados a su tamaño, cada salida se convierte en una experiencia inolvidable.
                            </p>
                            <p>
                                Es ágil, segura y perfecta para sus primeros desafíos off-road.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="relative hidden lg:flex items-center justify-center lg:justify-end">
                        <div className="relative w-full max-w-[700px]">
                            <img
                                src="https://www.raymon-bicycles.com/_next/image?url=https%3A%2F%2Fb2b.raymon-bicycles.com%2Fmedia%2Fd0%2Fbd%2Fa7%2F1747290585%2FFrame%20861.png&w=1920&q=75"
                                alt="Nayta Kids Adventure"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {bikes.length > 0 ? (
                        ...
                        <div className="col-span-full text-center text-gray-400 font-bold uppercase py-20">
                            Próximamente más modelos Nayta Kids.
                        </div>
                    )}
                </div>
            </div>
...
export default SeriesDetailNaytaKids;

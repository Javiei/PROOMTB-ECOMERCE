      ...
      content: (
        <div className="relative h-[80vh] min-h-[600px] overflow-hidden">
          ...
          <div className="relative z-10 h-full flex items-center">
            <div className="container mx-auto px-4">
              <div className="max-w-2xl text-white">
                <span className="inline-block px-4 py-2 rounded-full bg-orange-500 text-white font-bold mb-4 animate-bounce">
                  ¡NUEVO!
                </span>
                <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-4 leading-tight text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-orange-400">
                  ZONA KIDS
                </h2>
                <p className="text-lg md:text-xl mb-6 opacity-95 leading-relaxed font-medium">
                  Las mejores bicicletas para introducir a los pequeños en el mundo del ciclismo.
                  Seguridad, diversión y estilo para las futuras estrellas.
                </p>
                <button
                  onClick={() => navigate('/kids')}
                  className="bg-orange-500 text-white px-8 py-4 rounded-full font-bold text-lg hover:bg-orange-600 transition-all duration-300 transform hover:scale-105 shadow-lg flex items-center gap-2"
                >
                  Ver Colección Kids
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
      ...
      <div className="absolute bottom-8 left-0 right-0 z-20 flex justify-center space-x-2">
        {slides.map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full transition-all duration-300 ${currentSlide === index ? 'bg-white w-8' : 'bg-white/50 hover:bg-white/70'
              }`}
